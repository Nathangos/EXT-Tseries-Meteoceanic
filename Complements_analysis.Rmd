...
Residus_donnees<-read.csv(file=paste0("residuals/",Name_v,"_residuals.csv"))[,c(2:38)]
...
abline(v=Seuil,col="red")
par(mfrow=c(1,1))
mtext(paste0("Tcplot pour ",Name_v," en t=",t),outer=TRUE,line = -2,
      cex=1.3)
```
```{r}
...
colnames(df_coor_lien)<-c("value_data","value_simul",
                          "delta")
resultat_corr<-df_coor_lien %>% group_by(delta) %>% summarise(val_data=mean(value_data),
...
resultat_delta<-df %>% group_by(delta) %>% summarise(val_data=mean(value_data),
...
travail_extremo<-as.data.frame(resultat_delta[,c("val_data","val_simul","bound_inf","bound_sup")])
...
```{r}
# print result
GG_extremo<-ggplot(travail_extremo,aes(x=Temps,y=val_data,col="data"))+
  geom_line()+
  geom_point()+
...
```{r}
# ech pareto --------------------------------------------------------------
coords_donnees_Pareto<-read.csv(file=paste0("residuals/PCA_results/",Name_v,"_coords_obs.csv"))
...
resultat_obs<-read.csv(name_file)[,-1]

resultat_simul<-read.csv(name_file_simul)[,-1]
