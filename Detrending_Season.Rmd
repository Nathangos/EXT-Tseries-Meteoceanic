...
for(elt in annees){
  commentaire<-paste0("data/evt_",elt,"*.csv")
  file_year<- parLapply(cl=coeurs,Sys.glob(commentaire), read.csv)
  dataFiles<-c(dataFiles,file_year)
}
```
...
creation_df_sans_tendance<-function(df,nom_variable,coeurs,SELECT,saut_pas){
  ...
  TIME<-parLapply(cl=coeurs,df,f_nom_variable,"Date")
  intermed<-t(do.call(cbind.data.frame,TIME))
...
```{r}
Result_detrend_deseason<-creation_df_sans_tendance(df=dataFiles,nom_variable = "Surcote",coeurs=coeurs,SELECT=TRUE,saut_pas = 3)
```
```{r}
