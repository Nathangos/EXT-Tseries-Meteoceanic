...
Simulate_extreme_from_resid<-function(nb_decal,Sample_taken,
                                        obs,model_ar_per_t,j,Simul_epsi){
    index<-sort(c(Sample_taken[j])+c(-nb_decal:0),decreasing = TRUE)
    individu_taken<-obs[index,]
    Prediction<-as.numeric(sapply(X = c(1:ncol(obs)),FUN = Function_AR_p,obs_eve=individu_taken,epsilon_t_plus1=unlist(Simul_epsi[j,]),
                                  model_ar_per_t = model_ar_per_t))
    return(Prediction)
}
```
...
```{r}
name_variable<-"Surge"
fich_reg1<-paste0("residuals/",name_variable,"_Reg_AR1.csv")

file_Simulation_Residuals<-paste0("residuals/Simulations_",name_variable,"_with_std.csv")
...
Residuals<-read.csv(file =paste0("residuals/",name_variable,"_residuals.csv"))[,2:38]
colnames(Residuals)<-c(1:ncol(Residuals))

NL2_Residuals<-apply(X = Residuals,MARGIN = 1,FUN = calcul_norm_L2)
...
DF_EV_resid<-read.csv(file=paste0("Work_RVariations/EVA_",name_variable,".csv"))[,2:5]
...
```{r}
# model AR ---------------------------------------------------------------
model_ar_per_t<-read.csv(file=fich_reg1)
Fin<-ncol(model_ar_per_t)
model_ar_per_t<-model_ar_per_t[,c(2:Fin)]
...
NL2<-apply(X = obs,MARGIN = 1,FUN = calcul_norm_L2)
inds_non_null<-which(NL2_Residuals>0)
plot(NL2_Residuals[inds_non_null],NL2[inds_non_null],
...
par(mfrow=c(1,1))
N_XO<-c()
df_relation_inbase<-cbind.data.frame(NL2_Residuals[2:length(NL2_Residuals)],NL2_eve)
...
predictions_levels_passe<-predict(model_choix,data.frame(resid=NL2_epsi_simul))

```

```{r}
par(mfrow=c(1,2))
plot(density(NL2[Taken_samples]),main="norm L2 of obs (X0) takenes")
plot(density(NL2[Inds_extremes_L2-1]),main="norm L2 eve of obs extremes")
par(mfrow=c(1,1))
nb_decal<-0
if(ncol(model_ar_per_t)>2){
  nb_decal<-ncol(model_ar_per_t)-2
}
Y_Mat<-t(sapply(c(1:M),Simulate_extreme_from_resid,nb_decal = nb_decal,
                     Sample_taken = Taken_samples ,
                     model_ar_per_t = model_ar_per_t,Simul_epsi = Simulations_epsi,
                     obs = obs))
Y_Mat<-as.data.frame(Y_Mat)
colnames(Y_Mat)<-c(1:ncol(Y_Mat))
NL2_simulations<-apply(X =  Y_Mat,MARGIN = 1,FUN=calcul_norm_L2)
...
M_Prediction_obs_J<-t(sapply(c(1:nrow(Simulations_epsi)),
                            FUN = function(x)
    {vector<-as.numeric(sapply(X = c(1:ncol(obs)),FUN = Function_AR_p,obs_eve=eve_Johann,epsilon_t_plus1=unlist(Simulations_epsi[x,]), model_ar_per_t = model_ar_per_t))
     return(vector)}))
```

```{r}
## Simulate consecutive extremes
M_Prediction_consecutive_extreme<-t(sapply(c(1:nrow(Simulations_epsi)),FUN = function(x){vector<-as.numeric(sapply(X = c(1:ncol(obs)),FUN = Function_AR_p,obs_eve=Johann,             epsilon_t_plus1=unlist(Simulations_epsi[x,]),model_ar_per_t = model_ar_per_t))
return(vector)}))
```


```{r}
# versus normal observation -----------------------------------------------
...
```{r}
# Relation eve_XO vs norm epsilon -------------------------------------
...
```{r}
# Bootstrap confidence bands for data quantiles --------------------------------------------------
...
```{r}
# Data visualisation, comparing simulated and data levels --------------------------------------------------
### setup
...
```{r}
NL2_ext<-apply(X = Obs_ext,MARGIN = 1,FUN = calcul_norm_L2)
NL2_simul<-apply(X = Simul,MARGIN = 1,FUN = calcul_norm_L2)
```

```{r}
inds_non_null<-which(NL2_simul>0)
Angle_inds<-t(t(Obs_ext)%*%diag(NL2_ext^(-1)))
Angle_simul<-t(t(Simul[inds_non_null,])%*%diag(NL2_simul[inds_non_null]^(-1)))
...
plot(c(100,PCA_inds$eig[,2]),type="o"
     ,main=paste0("Evolution of the ratio of unexplained inertia for ",ifelse(name_variable=="Surge","S",name_variable)))
sd_angle<-apply(X =Angle_inds,MARGIN = 2,FUN = sd)
```

```{r}
#### Comparing marginal laws of simulated an observed PCA coordinates -- statistical tests. 
...
```{r}
# Data visualisation for comparing simulated and observed PCA coordinates
### setup
...
```{r}
## Computation for simulated time series and observed time series
...
## Boostrap confidence band for the extremogram
...
```{r}
# Data visualisation, comparing extremogram of simulated and observed extreme time series --------------------------------------------------
...
# confidence band for the extremogram
...
for(t in c(13,19,25,31)){
  ...
  name_graph<-ifelse(name_variable=="Surge","S",
                    name_variable)
...
```{r}
name_present<-ifelse(name_variable=="Surge",yes = "S",no = name_variable)
...
```{r}
## Run classification test to distinguished observed and simulated time series. 

##set-up
source("fonctions/fonctions_perfs_ML.R")
racine_ML<-c("results_ML/")

## The hyperparameters of each learner: the kernel of the SVM, the link function of the GLM and the number of trees of the random forest.
H_Params<-c("radial","logit",500)

## Number of repetitions of the experiment with the order of the confidence interval for the global accuracy.
NB_times<-100
ALPHA_PROP<-0.10
...
```{r}
## Classification test with the time series 
...
```{r}
## Classification test with the L2 norm of time series.
...
```{r}
## Classification test for the angle of time series
...
```{r}
## Printing the confidence intervals for the global accuracy for each case learner used
result
```
