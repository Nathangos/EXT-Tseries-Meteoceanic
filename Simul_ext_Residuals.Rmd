...
```{r}
Time_analysed<-c(19)
AN_GPD<-Analyse_threshd_GPD(data = Data,fonction_threshd = p_U,n.dens = n.dens,nom=name_variable,dates_prises=dates_import,j_show=Time_analysed)
...
Analyse_Pareto_par_time(data_Pareto = vector_pareto,nom = name_variable,n.dens=n.dens)
```
...
vector_norm_L2<-apply(X= vector_pareto,MARGIN = 1,FUN =calcul_norme_L2)
...
L2_proche_1<-apply(X =Shape_ACP,MARGIN = 1,FUN = calcul_norme_L2)
...
while (Gap!=0){
  ...
  Shape_Gap<-function_reconstitution_trajectory_std(Vector_coords = matrix_coords_Gap,Base_functions_p  = F_eigen,NB_dim = NB_dim_ACP,
                  mu_t =esp_time,sd_t = sd_time)
  
  L2_Gap<-apply(X =Shape_Gap,MARGIN = 1,FUN = calcul_norme_L2)
...
L2_realisations<-apply(X = New_obs_P_acp,MARGIN = 1,FUN = calcul_norme_L2)
...
mtext(xlab_chosen, side = 3, line = -22, outer = TRUE,
      cex=1.5)
# travail norme -----------------------------------------------------------
norme_theta_simul<-apply(X = Shape_for_analyse,MARGIN = 1,
                         FUN = calcul_norme_L2)
...
L2_X<-apply(X = Data[Inds_Exts,],FUN=calcul_norme_L2,MARGIN=1)
