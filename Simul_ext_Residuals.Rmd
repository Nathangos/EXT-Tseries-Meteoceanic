...
```{r}
name_variable<-"Surge"
link_data<-paste0("residuals/",name_variable,
                  "_residuals.csv")
std_data<-TRUE
p_U<-rep(0.10,37)
p_L2<-0.05
...
path_graph<-paste0(getwd(),"/Graphics/",name_variable)
try({
...
```{r}
Graphics_estimators_gamma(series = vector_norm_L2,vect_k = c(50:300),
...
```{r}
png(paste0(path_graph,"/Behaviour_PCA_",name_variable,"_1_2.png"))
plot(Analyse,choix = "var")
dev.off()
```

```{r}
png(filename = paste0(path_graph,"/Behaviour_PCA_",name_variable,"_2_3.png"))
plot(Analyse,choix = "var",axes = c(2:3))
dev.off()
```
...
```
```{r}
### Second best model for the first pair 
### Use Tawn instead of t copula for the first pair 
### Reminder) Asymptotic Dependence with the t copula 
### vs asymptotic independence in the pair of coordinates

### Using true make us use the second best model for the first pair
## of coordinates. 
SBT<-TRUE
if(SBT==TRUE){
  ...
  Sort_df<-head(DF[order(DF$AIC),c(1:5) ],5)
  Name<-Sort_df[2,1]
  Family_SB<-VineCopula::BiCopName(family = Name)
  
  ## Par, par2 from Model
  Theta_estim<-BiCopEst(dataUnif[,1],dataUnif[,2],
           family = Family_SB)
  par_ij<-Theta_estim$par
  par2_ij<-Theta_estim$par2
  
  ## Update_mod
  Par_mat<-Family_chosen$par
  Par2_mat<-Family_chosen$par2
  Fm_mat<-Family_chosen$family
  Par_mat[3,2]<-par_ij
  Par2_mat[3,2]<-par2_ij
  Fm_mat[3,2]<-Family_SB
  ## New object
  Family_chosen<-VineCopula::RVineMatrix(Matrix = matrix_C,
                family = Fm_mat
                ,par = Par_mat
                ,par2 = Par2_mat)
}
...
write.csv(x = Code_used,
  file=paste0("residuals/PCA_results/",
          name_variable,"_modele_Vine.txt"))
```
...
plot(dataUnif[,2],dataUnif[,3],xlab="Second coordinate (observations)",ylab="Third coordinate (observations)")
