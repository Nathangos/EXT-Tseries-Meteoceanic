...
```{r}
Reconstruction_trajectoire<-function(fichier_Simulation_residus,nom_variable,fich_reg1,fichier_idx_extremes,dates_Johanna){
  ...
  DF_EV_resid<-read.csv(file=paste0("Work_RVariations/EVA_",Nom_v,"_residus.csv"))[,2:5]
...
```{r}
Nom_v<-"Surcote"
name_for_file<-ifelse(Nom_v=="Surcote","S",no = Nom_v)
fich_reg1<-paste0("residuals/",name_for_file,"_Reg_AR1.csv")

fichier_simul<-paste0("residuals/Simulations_",Nom_v,"_avec_std.csv")
inds_extremes<-paste0("inds_extremes_donnees_",Nom_v,".csv")
DJohanna<-c("10/03/2008","08/03/2008", 
                 "09/03/2008")

```

```{r}
resultat_Simul_Obs<-Reconstruction_trajectoire(
      fichier_Simulation_residus =fichier_simul,nom_variable = Nom_v,
      fich_reg1 =fich_reg1,fichier_idx_extremes = inds_extremes ,
      dates_Johanna = DJohanna)
```
...
```{r}
nom_present<-ifelse(Nom_v=="Surcote",yes = "S",no = Nom_v)
...
plot(c(100,PCA_inds$eig[,2]),type="o"
     ,main=paste0("Evolution of the ratio of unexplained inertia for ",ifelse(Nom_v=="Surcote","S",Nom_v)))
...
Lperfs_Y<-Running_perfs_ML(Base_simul = Simul,Base_data = Obs_ext, 
                   ...
                   type_sampling = typeS,title_ROC =Nom_v,
                   NB_shown_ROC = 20)
...
Lperfs_L2<-Running_perfs_ML(Base_simul = NL2_simul,Base_data = NL2_ext, 
                           ...
                           ,title_ROC = paste0(Nom_v," Cost-f"),NB_shown_ROC = 20)
resultat[2,]<-Lperfs_L2$Accuracy$qu_accuracy
```

```{r}
Lperfs_A_f<-Running_perfs_ML(Base_simul = Angle_simul,Base_data = Angle_inds, hyp_param =H_Params , NB_times = NB_times,alpha_prop=ALPHA_PROP,K=K,type_sampling = typeS, title_ROC = paste0(Nom_v," angle"),NB_shown_ROC = 20)
...
Indices_exts<-read.csv(paste0("residuals/",inds_extremes))[,2]
